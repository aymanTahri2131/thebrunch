...
export const Navigation = () => {
  ...
  const navLinks: NavLink[] = [
    ...
    { name: "FAQ", path: "/questions" }, // ← هنا زدنا FAQ
  ];
      ...
      <div className="container mx-auto px-4">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => {
              if (link.scrollTo) return (
                <button key={link.name} onClick={() => scrollToSection(link.scrollTo)} className="text-foreground hover:text-primary transition-colors font-medium">{link.name}</button>
              );
              if (link.isContact) return (
                <button key={link.name} onClick={handleContactClick} className="text-foreground hover:text-primary transition-colors font-medium">{link.name}</button>
              );
              if (link.isHome) return (
                <button key={link.name} onClick={handleHomeClick} className="text-foreground hover:text-primary transition-colors font-medium">{link.name}</button>
              );
              return (
                <Link key={link.path} to={link.path} className="text-foreground hover:text-primary transition-colors font-medium">{link.name}</Link>
              );
            })}
        ...
        <div className={cn("md:hidden overflow-hidden transition-all duration-300", isOpen ? "max-h-96 pb-4" : "max-h-0")}>
          <div className="flex flex-col space-y-4 pt-4">
            {navLinks.map((link) => {
              if (link.scrollTo) return (
                <button key={link.name} onClick={() => { scrollToSection(link.scrollTo); setIsOpen(false); }} className="text-foreground hover:text-primary transition-colors font-medium py-2 text-left">{link.name}</button>
              );
              if (link.isContact) return (
                <button key={link.name} onClick={() => { handleContactClick(); setIsOpen(false); }} className="text-foreground hover:text-primary transition-colors font-medium py-2 text-left">{link.name}</button>
              );
              if (link.isHome) return (
                <button key={link.name} onClick={() => { handleHomeClick(); setIsOpen(false); }} className="text-foreground hover:text-primary transition-colors font-medium py-2 text-left">{link.name}</button>
              );
              return (
                <Link key={link.path} to={link.path} onClick={() => setIsOpen(false)} className="text-foreground hover:text-primary transition-colors font-medium py-2 text-left">{link.name}</Link>
              );
            })}
