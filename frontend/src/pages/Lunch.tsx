...
const Lunch = () => {
  ...
  const visibleCategories = categories.slice(0, 3); // Limiter à 3 catégories pour l'affichage
  ...
  {/* Content */}
      <div className="relative z-10 container mx-auto px-4 text-center text-white">
        ...
        <p className="text-sm md:text-base mb-8 text-white/90 max-w-2xl mx-auto" style={{ fontFamily: '"Inconsolata", monospace' }}>
          Découvrez une sélection exquise de créations salées et sucrées, ainsi que des brunchs d’exception imaginés pour sublimer vos réceptions. Chaque détail est pensé pour émerveiller vos convives et transformer vos instants gourmands en expériences culinaires uniques et mémorables.
        </p>
        </div>
      </section>
                  ...
                  {allCategories.slice(3).map((category) => {
                    ...
                    return (
                      <TabsTrigger
                        key={category.id}
                        value={category.id}
                        className="flex flex-col gap-2 p-3 mx-1 data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#cbb36f] data-[state=active]:to-[#99771b] data-[state=active]:text-white rounded-xl transition-all duration-300"
                      >
