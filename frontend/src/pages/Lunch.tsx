              ...
              <div className="text-center mb-8">
                <TabsList className="grid w-full max-w-6xl mx-auto mb-12 h-48 p-2 bg-accent/10 border border-accent/30 shadow-lg rounded-2xl" style={{ gridTemplateColumns: `repeat(${Math.min(visibleCategories.length, 3)}, 1fr)` }}>
                  {allCategories.slice(3).map((category) => {
                    const IconComponent = getIconForCategory(category.id);
                    return (
                      <TabsTrigger key={category.id} value={category.id} className="flex flex-col gap-2 p-4 py-16 mx-1 data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#cbb36f] data-[state=active]:to-[#99771b] data-[state=active]:text-white rounded-xl transition-all duration-300">
